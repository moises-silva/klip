...
def test_strikethrough_mid_sentence():
    assert (
        md_to_chat("event ~~cancelled~~ rescheduled") == "event ~cancelled~ rescheduled"
    )
...
def test_link_with_surrounding_text():
    assert (
        md_to_chat("See [docs](https://example.com) for details")
...
def test_bare_url_unchanged():
    assert (
        md_to_chat("Visit https://example.com now") == "Visit https://example.com now"
    )
...
def test_bare_email_converted_to_gmail_compose():
    ...
    assert (
        result
        == "from <https://mail.google.com/mail/?view=cm&to=moy@example.com|moy@example.com> today"
    )
...
def test_bare_email_at_end_of_sentence_no_trailing_dot():
    ...
    assert (
        result
        == "Contact <https://mail.google.com/mail/?view=cm&to=alejandro@example.com|alejandro@example.com>. Is this correct?"
    )
...
def test_bare_url_in_code_span():
    ...
    result = md_to_chat(
        "Links to `https://modelcontextprotocol.io/docs/learn/architecture`"
    )
    assert (
        result
        == "Links to <https://modelcontextprotocol.io/docs/learn/architecture|https://modelcontextprotocol.io/docs/learn/architecture>"
    )
...
def test_link_and_bold_combined():
    assert (
        md_to_chat("**[notes](https://docs.google.com)**")
